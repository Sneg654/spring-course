...
public class CounterAspect {
    ...
    @Pointcut("(execution(* com.epam.beans.services.EventService.getEvent(String, ..)) && args(eventName, ..)) || "
              + "execution(* com.epam.beans.services.EventService.getByName(String)) && args(eventName))")
    private void accessedByName(String eventName) {
    }

    @Pointcut("(execution(* com.epam.beans.services.BookingService.getTicketPrice(String, ..)) && args(eventName,..))")
    private void getPriceByName(String eventName) {
    }

    @Pointcut("(execution(* com.epam.beans.services.BookingService.bookTicket(*, com.epam.beans.models.Ticket)) && args(*, ticket))")
